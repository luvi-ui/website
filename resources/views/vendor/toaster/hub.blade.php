...
>
    ...
    >
        ...
        @else
            ...
            @class([
                'relative duration-300 transform transition ease-in-out max-w-xs w-full pointer-events-auto  group flex items-center justify-between space-x-2 overflow-hidden rounded-md border p-4 pr-6 shadow-lg',

                'text-center' => $position->is('center'),
            ])
            :class="toast.select({ error: 'text-white', info: 'text-black', success: 'text-black', warning: 'text-black' })"
            >
            <p
                x-text="toast.message"
                class="text-sm opacity-90 {{ $alignment->is('bottom') ? 'mt-3' : 'mb-3' }}"
            ></p>
